...
export default function Register({ onClose, onSuccess }) {
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      const session = { id: data.user.id, phone: formData.phone, role: formData.role };
      localStorage.setItem("shramik_user", JSON.stringify(session));
      onSuccess({ id: data.user.id, ...formData });
      onClose();
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl relative">
        ...
        {step === 'form' && (
          <>
            ...
            <form onSubmit={sendOtp} className="p-6 pt-4 space-y-3">
              {error && (
              ...
              <div className="relative">
                ...
                <input type="text" name="full_name" placeholder={t('reg_name_placeholder')} value={formData.full_name} onChange={handleChange} required maxLength={100}
              ...
              >
                {isLoading ? (
                  ...
                  <><MessageSquare size={18} /> Send OTP</>
                )}
              </button>
        ...
        {step === 'otp' && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">Enter the 6-digit code sent to {formData.phone}</p>
            </div>
            ...
            <p className="text-xs text-center text-slate-400">
              Didn't receive OTP?{" "}
              <button onClick={resendOtp} disabled={isLoading || cooldown > 0} className="text-saffron font-bold hover:underline">
